...
Habria que ver que hacer con la variable edad, tiene sentido centralizarla? 
...
data = data %>%  mutate(momento = case_when(momento == "previo" ~ 0,
              ...
              domicilio = as.factor(domicilio),
                        sexo = as.factor(sexo),
              id = as.factor(id), edad = edad - mean(edad)) %>% filter(id!=53)  
...
mod2 = lmer(barthel ~   + tratamiento + (1|id),data= data, REML =FALSE)
