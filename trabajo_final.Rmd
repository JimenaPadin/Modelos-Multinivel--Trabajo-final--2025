...
A continuación, se presenta un segundo gráfico con la evolución en el tiempo de cinco de los pacientes. Se identifica que todos comparten el mismo punto inicial, donde la tasa de recuperación al ingreso en el hospital vale 0, esto tiene sentido porque es el momento en el que sufrieron el traumatismo, previo a comenzar el tratamiento.
...
*hablar de porque no incorpotrramos barthel*

Por lo que las variables a seleccionar son:

*esta raro, capaz va a en otro lado, tipo armarlo si como tabla despues de las correlaciones*

```{r}

data_frame("Variable"=c("Recuperación", "Tratamiento", "domicilio", "sexo", "momento2", "edad2", "b_ini", "b_prev"), 
           "Interpretación"=c("Variable dependiente", "Variable categórica que indica el típo de tratamiento que es sometido el paciente luego de la lesión, esta es una variable de nivel 2 ya que este no cambia en el tiempo.",  "Variable dummy que indica si el individuo hizo la recuperacion en su domicilio (si) o en un centro de reahbilitación (no). Esta es una variable de nivel 1.", "", "variable que toma como referencia el primer mes luego del ingreso, la metrica temporal queda definida como: [-1, 0, 2, 11, 23], es una variable de nivel 1 ya que esta variable cambia en el tiempo.", "variable de edad de los individuos a la hora del ingreso , centrada en 60 años.", "Indice de Barthel al ingreso al hospital.", "Indice de Barthel previo a la lesion.")) %>% 
  kable(caption = "Variables utilizadas en los modelos", booktabs = TRUE) %>%
  kable_styling(
...
-   **domicilio**: Variable dummy que indica si el individuo hizo la recuperacion en su domicilio (si) o en un centro de reahbilitación (no). Esta es una variable de nivel 1.
...
-   **b_ini**: Indice de Barthel al ingreso al hospital.

-   **b_prev**: Indice de Barthel previo a la lesion.

Se decidió no incorporar la variable Barthel al modelo debido a que como se mencionó anteriormente, esta se utiliza para construir la variable dependiente, por lo que el incluirla en el modelo llevaría a que absorba el efecto de las demás variables y se generen interpretaciones erróneas.
...
data |>
  group_by(momento2) |>
  summarise(r_barthel =cor(barthel,recuperacion,use = "complete.obs")) %>% mutate(r_barthel=round(r_barthel,2)) %>% rename("correlación"=r_barthel) %>% 
  kable(caption = "Correlación entre barthel y recuperación", booktabs = TRUE) %>%
  kable_styling(
    bootstrap_options = c("striped", "hover"),
    full_width = FALSE,
    position = "center"
...
Si bien la prueba sugiere que el modelo con pendiente aleatoria mejora el ajuste, el ajuste del modelo más complejo presenta un problema de *boundary (singular) fit*.
...
Bajo estas condiciones, el modelo con pendiente aleatoria no es identificable y los valores del log-likelihood utilizados en el LRT no son confiables. *cantidad obs x id muy baja*
...
compare_performance(mod3, mod4, rank = TRUE)
```

*cambiar queda mod4* En función de las pruebas realizadas y del problema de singularidad observado al intentar estimar pendientes aleatorias, se adopta como estructura aleatoria final un modelo con intercepto aleatorio y pendiente fija para `momento2`. El modelo jerárquico resultante es:

$$recuperacion_{it} = \beta_{0i} + \beta_{1i}momento2_{it} + \varepsilon_{it} \qquad i=1,2,...,327\quad t=-1,0,2,11,23$$ $$\beta_{0i}=\gamma_{00}+\eta_{0i}$$

```{r, echo=T}
mod0 <- lmer(recuperacion ~ 1 + (0+momento2|id), data=data)
...
```{r}
mod1 = lmer(recuperacion ~ sexo + edad2 + domicilio + (0+momento2|id), data= data)
...
*cambiar, se da a entender que el anterior tenia tratamiento*
...
```{r}
mod2= lmer(recuperacion ~ edad2 + domicilio + tratamiento + (0+momento2|id),data= data) #sacamos sexo y agregamos tratamineto, comparamos con mod0
...
```{r,eval=F, echo=T}
mod3= lmer(recuperacion ~ edad2 + domicilio + tratamiento + b_prev + (0+momento2|id),data= data)
anova(mod2,mod3) #se acepta el nuevo modelo
summary(mod3, ddf = "Kenward-Roger")
```

*no vale la pena, 95* *probamos b_ini*

```{r,eval=F, echo=T}
mod4= lmer(recuperacion ~ edad2 + domicilio + tratamiento + b_prev + b_ini + (0+momento2|id),data= data)
...
newdata <- 
   data.frame(
      id=c(rep(328,5),rep(329,5)),
      sexo=c('M','M','M','M','M', 'F', 'F', 'F', 'F', 'F'),
      edad2=c(rep(85-60, 10)),
      tratamiento = c(rep('cirugía',5), rep('fisioterapia',5)),
      domicilio=c(rep('si',10)),
      momento2=c(-1,0,2,11,23,-1,0,2,11,23),
      b_prev=c(rep(65,10)))

newdata$recuperacion <- predict(object=mod_final, newdata=newdata)
