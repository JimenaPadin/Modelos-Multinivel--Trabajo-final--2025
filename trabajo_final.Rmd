...
```
Veamos la variablidad de edades para esta muestra mediante un histograma, vemos que la mayoria de las personas que ingresaron al tratamiento tienen entre  80 y 90 años 
...
```{r}
mod2= lmer(recuperacion ~ barthel + edad +  tratamiento+ (momento2|id),data= data)
summary(mod2, ddf = "Kenward-Roger")
```
...
```{r}
mod3 <- lmer(recuperacion ~ barthel + momento2 + tratamiento +
                            momento2:tratamiento +
                            edad + (momento2 | id),
             data = data)
summary(mod3, ddf = "Kenward-Roger")
```
...
#MODELO FINAL)?
mod4= lmer(recuperacion ~ barthel*momento2  + edad + (1|id),data= data)
summary(mod4, ddf = "Kenward-Roger")
