...
- **momento**: Variable que toma valores: "previo", "ingreso","1 mes", "3 meses", "12 meses" y "24 meses"

- **barthel** variable continua  que mide la capacidad funcional del indiivudo, va desde 0 hasta 100. Variable de nivel tipo 1 ya que varia en el tiempo. 

- **edad** variable de edad de los individuos al momento del ingreso. Variable de nivel 2. 
...
- **b_ini**: Indice de Barthel al final de la lesión
...
Se comienza por discernir la estructura apropiada de la matriz $\pmb D$. Bajo la hipótesis, se asume que ningún efecto aleatorio $k$ tiene un aporte significativo al modelo (modelo lineal):
...
Bajo estas condiciones, el modelo con pendiente aleatoria no es identificable y los valores del log-likelihood utilizados en el LRT no son confiables.
...
$$recuperacion_{ij} = \beta_{0i} + \beta_{1i}momento2_{ij} + \varepsilon_{ij}$$
$$\beta_{0i}=\gamma_{00}+\eta_{0i}$$
$$\beta_{1i}=\gamma_{10}$$

En el Anexo X se muestra que agregar otras variables como pendientes aleatorias no aporta mejoras significativas al modelo.

$$R_{ij} = \beta_{0i} +\beta_{1i}momento2_{ij} + \epsilon_{ij} \rightarrow \ ecuación\  de\  nivel\ 1$$ 
donde:

$$\beta_{0i}=\gamma_{00}+\eta_{0i} \rightarrow ecuación\ de\ nivel\ 2\ para\ la\ constante$$ 

La interpretación del $gamma_{00}$ seria la recuperación media de la población en el primer mes del ingreso. 


$$\beta_{1i}=\gamma_{10} \ ecuación \ de \ nivel\ 2 $$
...
## Inclusión de covariables al modelado
...
```{r,eval=F, echo=T}
mod3= lmer(recuperacion ~ momento2 + edad2 + domicilio + tratamiento + barthel + (1|id),data= data)
anova(mod2,mod3) #se acepta el nuevo modelo
summary(mod3, ddf = "Kenward-Roger")
```

_tratamiento pierde relevancia, momento2 y domicilio son menos significativos_

_como barthel es de nivel 2 no convendría agregarla solo como interaccion?_
...
```{r,eval=F, echo=T}
mod4= lmer(recuperacion ~ momento2 + edad2 + domicilio + tratamiento + barthel*momento2 + (1|id),data= data)
mod5= lmer(recuperacion ~ momento2 + edad2 + domicilio + tratamiento + barthel*momento2 + barthel + (1|id),data= data)
anova(mod4,mod5) #evaluo si es mejor barthel solo como inetarccion o ambas, SON LO MISMO?
```
...
_anterior de luciana que no movi todavia_
...
$$R_{ij} = \beta_{0i} + (\beta_{1i}Tratamiento_{ij})*(Momento2_{ij}) + \beta_{2i}edad_{ij} + \epsilon_{ij}$$
$$\beta_{0i} = \gamma_{00} +\eta_{0i}$$
